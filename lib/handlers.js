...
handlers._users = {
  post: (data, callback) => {
    handlers._users._hydratePayload.post(data.payload, (payload) => {
  ...
  get: (data, callback) => {
    ...
    if (phone) {
      ...
      handlers._tokens._verifyToken(token, phone, (isTokenValid) => {
        if (isTokenValid) {
          _data.read('users', phone, (err, data) => {
            if (!err && data) {
  ...
  put: (data, callback) => {
    handlers._users._hydratePayload.put(data.payload, (payload) => {
  ...
  // @TODO Cleaning (delete) ant other data files associated with this user.
...
handlers._tokens = {
  post: (data, callback) => {
    handlers._tokens._hydratePayload.post(data.payload, (payload) => {
      ...
      if (phone && password) {
        _data.read('users', phone, (err, userData) => {
          if (!err) {
            ...
            callback(400, { 'Error': 'Could not find the sspecified user' });
          }
        });
      } else {
  ...
  put: (data, callback) => {
    handlers._tokens._hydratePayload.put(data.payload, (payload) => {
  ...
  _verifyToken: (id, phone, callback) => {
    _data.read('tokens', id, (err, tokenData) => {
...
handlers._checks = {
  post: (data, callback) => {
    handlers._checks._hydratePayload.post(data.payload, (payload) => {
          ...
          if (!err && tokenData) {
            ...
            _data.read('users', userPhone, (err, userData) => {
              if (!err && userData) {
                ...
                userData.checks = Array.isArray(userData.checks) ? userData.checks : [];
    ...
    if (id) {
      _data.read('checks', id, (err, checkData) => {
        if (!err && checkData) {
          ...
          handlers._tokens._verifyToken(token, userPhone, (isTokenValid) => {
            if (isTokenValid) {
  ...
  put: (data, callback) => {
    ...
    if (id) {
      handlers._checks._hydratePayload.put(data.payload, (payload) => {
        const { protocol, url, method, successCodes, timeoutSeconds } = payload;
        if (protocol || url || method || successCodes || timeoutSeconds) {
          _data.read('checks', id, (err, checkData) => {
            if (!err && checkData) {
  ...
  _hydratePayload: {
    post: (payload, callback) => {
      ...
      const timeoutSeconds = isNumberNatural(timeout) && timeout <= maxChecks ? timeout : false;
      callback({ protocol, url, method, successCodes, timeoutSeconds });
    },
    put: (data, callback) => {
      handlers._checks._hydratePayload.post(data, callback);
    },
  },
};
